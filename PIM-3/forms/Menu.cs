...
namespace SistemaBibliotecaBonito
{
    ...
    {
        private List<Control> dashboardOriginal;
        ...
        public void AtualizarTotais()
        {
            ...
            label5.Text = Banco.Usuarios.Count.ToString();

            int emprestados = Banco.Emprestimos.Count(emp => emp.Devolvido == false);
            label13.Text = emprestados.ToString();

            int atrasados = Banco.Emprestimos.Count(emp =>
                emp.Devolvido == false &&
                DateTime.ParseExact(emp.DataDevolucao, "dd/MM/yyyy", CultureInfo.InvariantCulture).Date < DateTime.Now.Date
            );

            label14.Text = atrasados.ToString();
        }
